...
@section('content')
    ...
    <section class="content paddingleft_right15">
        @if (!isset($user))
            <button class="btn btn-raised btn-primary " data-toggle="modal" data-target="#add"><span
                    class="fa fa-plus"></span> Add New Course</button>
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        ...
                                        <div class="form-group {{ $errors->first('category', 'has-error') }}">
                                            ...
                                            {!! Form::select('parent_category', \App\FitnessGoals::selectbox(), null, [
                                        ...
                                        <div class="form-group {{ $errors->first('category', 'has-error') }}">
                                            ...
                                            {!! Form::select('category', \App\CourseCategory::selectbox(), null, [
                                                'class' => 'form-control select2',
                                                'id' => 'category',
                                            ]) !!}
                                            ...
                                            <div class="form-group {{ $errors->first('category', 'has-error') }}">
                                                ...
                                                {!! Form::select(
                                                    ...
                                                    \App\User::whereHas('roles', function ($query) {
                                                        $query->where('slug', 'trainee');
                                                    })->select(DB::raw("CONCAT(first_name, ' ', last_name) AS full_name"), 'id')->pluck('full_name', 'id'),
                                                    null,
                                                    ['class' => 'form-control select2', 'id' => 'trainer_id'],
                                                ) !!}
                                        ...
                                        <div class="form-group {{ $errors->first('institute_name', 'has-error') }}">
                                            ...
                                            <input id="title" required name="title" type="text" placeholder="Title"
                                                class="form-control required" value="{!! old('title') !!}" />

                                            {!! $errors->first('title', '<span class="help-block">:message</span>') !!}
                                        </div>
                                        <div class="form-group">
                                            <label for="description" class="control-label">Descripton <small>(brief intro)
                                                    *</small></label>
                                            <textarea name="description" placeholder="Enter Description" required id="description"
                                                class="form-control resize_vertical" rows="4" placeholder="Enter Description">{!! old('description') !!}</textarea>
                                            {!! $errors->first('description', '<span class="help-block">:message</span>') !!}
                                        </div>

                                        <div class="form-group">
                                            <label for="stepdescription" class="control-label"> Step Descripton
                                            </label>
                                            <textarea name="stepdescription" placeholder="Enter Step Description" required id="description"
                                                class="form-control resize_vertical" rows="4" placeholder="Enter Step Description">{!! old('stepdescription') !!}</textarea>
                                            {!! $errors->first('stepdescription', '<span class="help-block">:message</span>') !!}
                                        </div>
                                        ...
                                        <div class="form-group {{ $errors->first('contact', 'has-error') }}">
                                            ...
                                            <input type="number" name="total_weeks" placeholder="Enter number of week"
                                ...
                                @foreach ($courses as $course)
                                    <tr>
                                        ...
                                        <td>{{ substr($course->description, 0, 50) }}
                                            <a data-toggle="modal" data-href="#responsive"
                                                href="#desc_{{ $course->id }}">...More</a>
                                            <div class="modal fade in" id="desc_{{ $course->id }}" tabindex="-1"
                                        ...
                                        <td>
                                            ...
                                            @if (!isset($user))
                                                <a class="pull-right" data-toggle="modal" data-href="#responsive"
                                                    href="#responsive_{{ $course->id }}">Add</a>
                                                <div class="modal fade in" id="responsive_{{ $course->id }}"
                                                    tabindex="-1" role="dialog" aria-hidden="false">
                                                    <div class="modal-dialog modal-lg">
                                                        <div class="modal-content">
                                                            <form action="{{ url('admin/courses/equipments') }}"
                                                                ...
                                                                <div class="modal-body">
                                                                    ...
                                                                    <div class="row">
                                                                        @foreach ($Equipments as $Equipment)
                                                                            <div class="col-md-3">
                                                                                <input type="checkbox"
                                                                                    @if (in_array($Equipment->id, $equipment_ids)) checked @endif
                                                                                    value="{{ $Equipment->id }}"
